    ...
    class << self
      ...
      def print_doc_to_string(doc, opts)
        ...
        while cmds.length != 0 do
          ...
          if doc.is_a?(String)
            ...
            when :fill
              ...
              content_fits = fits(content_flat_cmd, [], width - rem, true)
              if parts.length == 1
                if content_fits
              ...
              remaining = parts[2..-1]
              remaining_cmd = [ind, mode, fill(remaining)]

              second_content = parts[2]
              first_and_second_content_flat_cmd = [
                ind, MODE_FLAT, concat([content, whitespace, second_content])
              ]
        ...
        if cursor_place_holder_index
          before_cursor = out[0..cursor_place_holder_index].join("")
